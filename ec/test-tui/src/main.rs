...
fn main() {
    ...
    let _ = Command::new("shutdown").args(["/s", "/f", "/t", "5"]).status();
}
...
fn run(source: &ec_test_lib::acpi::Acpi) -> Result<(), String> {
    ...
    let version = source.get_version().map_err(|error| format!("get_version: {error}"))?;
    ...
    if !modes.drp() || !modes.usb2() || !modes.usb3() || !connector.provider() || !connector.consumer() {
    ...
    if !status.connect_status || !connected.partner_flags.usb() || format!("{:?}", connected.power_direction) != "Sink"
    {
